...
def show_song(song_id):
    ...
    return render_template("song.html", song=song, playlists=song_in_playlists)
...
def add_song():
    ...
    if form.validate_on_submit():
        name = form.name.data
        description = form.description.data
        new_song = Song(name=name, description=description)
...
def add_song_to_playlist(playlist_id):
    ...
    songs_not_on_playlist = (
        db.session.query(Song.id, Song.name)
        .filter(Song.id.notin_(curr_on_playlist))
        .all()
    )

    form.song.choices = [(song.id, song.name) for song in songs_not_on_playlist]
